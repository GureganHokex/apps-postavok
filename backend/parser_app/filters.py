...
class ContactFilter:
    ...
    def filter_row(self, row_data: Dict, row_text: str = '') -> Tuple[bool, Dict]:
        ...
        if contacts or has_service_keywords:
            # Проверяем, содержит ли строка контактную информацию (телефон, email, ссылки)
            contact_indicators = ['@', 'http', 'www.', 'тел', 'телефон', 'phone', '+7', '+375', 'telegram', 't.me', 'instagram', 'vk.com']
            has_contact_indicators = any(indicator in row_text_lower for indicator in contact_indicators)
            
            # Если строка содержит контакты и нет товарных данных - это служебная строка
            if has_contact_indicators and not has_product_data:
        ...
        # Минимальная проверка: пропускаем только явно неправильные названия
        beer_name = row_data.get('beer_name', '').strip().lower() if row_data.get('beer_name') else ''
        if beer_name:
            # Пропускаем если название - это заголовок
            if beer_name in ['название', 'наименование', 'наличие', 'наличии']:
                return False, contacts
            
            # Пропускаем если название слишком короткое (меньше 3 символов)
            if len(beer_name) < 3:
                return False, contacts
