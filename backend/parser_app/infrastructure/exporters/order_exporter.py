...
from parser_app.models import Order, ParsedItem, TapLocation, Tap
import pandas as pd
import copy
    ...
    def export_to_excel(self) -> str:
        ...
        for order_item in self.order.items:
            ...
            try:
                ...
                if location:
                    # Для виртуальных вкладок (Банки/Кеги у частных поставщиков) используем original_sheet
                    sheet_name = location.get('original_sheet') or location.get('sheet')
        ...
        for sheet_name in wb.sheetnames:
            ...
            for item in self.order.items:
                ...
                try:
                    parsed_item = ParsedItem.objects.get(id=item_id)
                    location = parsed_item.raw_source_location
                    sheet_for_export = (location.get('original_sheet') or location.get('sheet')) if location else None
                    if location and sheet_for_export == sheet_name:
                        row_num = location.get('row')
                        excel_row = row_num + 2
