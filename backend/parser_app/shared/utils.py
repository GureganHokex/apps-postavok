...
def normalize_number_str(value):
    """
    Нормализует строку с числом для парсинга (прайсы: запятая/точка, пробелы).
    Примеры: "1 234,56" -> "1234.56", "1.234,56" -> "1234.56".
    
    Args:
        value: Строка или число
        
    Returns:
        Строка с точкой как десятичным разделителем без пробелов, или исходное значение.
    """
    if value is None:
        return None
    ...
    if ',' in s and '.' in s:
        # Точка — тысячи, запятая — десятичная
        s = s.replace('.', '').replace(',', '.')
    elif ',' in s:
