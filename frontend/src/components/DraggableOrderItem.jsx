...
function DraggableOrderItem({ item, index, onQuantityChange, quantity, quantityErrors, getItemTotal }) {
  ...
  // Сумма по строке с учётом логики кег
  const rowTotal = getItemTotal
    ? getItemTotal(item, quantity)
    : quantity * (parseFloat(item.price) || 0);
  ...
  return (
    ...
    >
      ...
      <td className="sum">{rowTotal.toFixed(2)}</td>
    </motion.tr>
  );
}
